...
mkdir proteomics/metamorpheus_semi
metamorpheus_dir=proteomics/metamorpheus_semi

types=("drug_product" "lysate")

for type in "${types[@]}"; do 

    mkdir $metamorpheus_dir/$type

    if [ $type == "drug_product" ]; then
        
        studies=("pythoud" "tzani")
    ...
    for study in "${studies[@]}"; do
        ...
        for sample in "${samples[@]}"; do
            ...
            if [ $study == "tzani" ] && [ $type == "drug_product" ]; then
                ...
                metamorpheus \
                -t data/metamorpheus/Task1-SearchTaskconfig_reducing_semi.toml \
                data/metamorpheus/Task2-CalibrateTaskconfig_reducing_semi.toml \
                data/metamorpheus/Task3-SearchTaskconfig_reducing_semi.toml \
                -s $raw_dir \
                ...
                metamorpheus \
                -t data/metamorpheus/Task1-SearchTaskconfig_"$prep_type"_semi.toml \
                data/metamorpheus/Task2-CalibrateTaskconfig_"$prep_type"_semi.toml \
                data/metamorpheus/Task3-SearchTaskconfig_"$prep_type"_semi.toml \
                -s $raw_dir \
