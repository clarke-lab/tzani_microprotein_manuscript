...
volcano_plot_data <- function(deseq_unfiltered) {
  vp_data <- deseq_unfiltered %>%
    as_tibble(rownames = "geneid") %>%
    drop_na(padj) %>%
    mutate(diff_rnaseq = case_when(
      log2FoldChange >= log2(fold_change_threshold) & padj < pval_threshold ~ "Up",
      log2FoldChange <= -log2(fold_change_threshold) & padj < pval_threshold ~ "Down",
      TRUE ~ "No change"
    )) %>%
...
facet_zoom2 <- function(x, y, xy, zoom.data, xlim = NULL, ylim = NULL, 
  ...
  ggplot2::ggproto(NULL, FacetZoom2,
          shrink = shrink,
          params = list(
...
# scale coverage between 0 and 1
