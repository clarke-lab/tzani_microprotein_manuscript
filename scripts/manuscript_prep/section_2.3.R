...
paste0("___________________________________________________________________________________")
paste0("                        smORF category by start codon                              ")
paste0("___________________________________________________________________________________")
orftable %>%
  filter(`ORF type` == "Upstream" | `ORF type` == "Start overlap" | `ORF type` == "New") %>%
  mutate(category = case_when(
...
new_orf_cds <- extractTranscriptSeqs(cgr_fasta, nc_orf_granges)
...
proportion_sorf <- orftable %>%
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
proportion_sorf
# ggsave(plot = proportion_sorf, filename = paste(results_dir, "Figure 3b.png", sep = ""), 
#        width = 3, height = 3.5, device = "png", dpi = 2000)


## ------------------------------------------------------------------------------------------------------------
# create fasta files for microprotein categories
txdb_orf <- loadTxdb("orf_identification/orf_filtered/orfrater_annotation.gtf.db",
                     chrStyle = NULL)
...
orf_to_fasta(upstream_sorf, cds_novel_orfs, fasta_out_file, cgr_fasta)
...
orf_to_fasta(lncrna_sorf, cds_novel_orfs, fasta_out_file, cgr_fasta)
...
fasta <- readAAStringSet("reference_genome/GCF_003668045.3_CriGri-PICRH-1.0_protein.faa")
...
# account for the different encoding of the CDS on NCBI
# canonical_CDS_start <- XM_027418155.2[XM_027418155.2$"ORF type" == "Annotated","Transcript start position"]-1
# canonical_CDS_stop <- XM_027418155.2[XM_027418155.2$"ORF type" == "Annotated","Transcript stop position"]
