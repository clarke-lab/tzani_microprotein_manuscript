...
reference_annotation_plastid_merged <- read_delim(paste0(root_dir, "results/diff_trans_analysis/plastid_reference/annotation_merged.txt"),
  "\t",
...
te_sample_table <- te_sample_table %>%
  arrange(assay)
...
ddsMat$condition <- relevel(ddsMat$condition, ref = "nts")
ddsMat$assay <- relevel(ddsMat$assay, ref = "rnaseq")


ddsMat = DESeq(ddsMat)

res_te = results(ddsMat, name="conditionts.assayriboseq")

dds_te <- DESeq(dds_te)
res_te <- results(dds_te, name = "conditionts.assayriboseq")
resultsNames(ddsMat )
